...
- `chambaflow/cli.py::print_welcome_banner()` tiene 3 niveles de layout según `console.size.width` (side-by-side mascota+wordmark / apilado / compacto sin wordmark ASCII) porque el wordmark ASCII mide 86 cols — en terminales angostas (~80 cols, default en muchas consolas Windows) rich lo envolvía carácter por carácter y salía ilegible. Si se edita el ASCII art, revalidar los 3 anchos (ver umbral `wordmark_width + mascot_width + 3` y `wordmark_width` en el código).
